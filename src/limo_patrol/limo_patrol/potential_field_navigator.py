...
class PotentialFieldNavigator(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            "goal_tolerance", 0.3
        )  # Tolerancia para alcanzar el waypoint
        ...
        self.mission_completed = False  # flag para que no se reinicie
    ...
    def control_loop(self):
        ...
        # 0. Chequeo si se alcanzaron todos los waypoints
        ...
        if abs(yaw_error) > 1.2:
            ...
            linear_vel = min(force_magnitude, self.max_linear_speed) * math.cos(
                yaw_error
            )
            linear_vel = max(0.05, linear_vel)

        angular_vel = yaw_error * 1.5
        angular_vel = max(
